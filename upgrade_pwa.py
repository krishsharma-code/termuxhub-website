...
for filename in html_files:
    ...
    # Remove existing SW registration if it's the old one (especially in index.html)
    old_sw_pattern = r"// Register Service Worker\s*if \('serviceWorker' in navigator\) \{.*?\}\s*\}"
    content = re.sub(old_sw_pattern, "", content, flags=re.DOTALL)
    
    # Also remove any old registration that uses service-worker.js
    content = re.sub(r"navigator\.serviceWorker\.register\('service-worker\.js'\);?", "", content)

    # Inject new script logic before </body> if not already present
    if 'id="installAppBtn"' in content and 'PWA Install Logic' not in content:
